...
abstract sealed class _AbstractMemorySegmentImpl
    ...
    private static Runnable cleanupAction(long address, long newSize, Consumer<MemorySegment> cleanup) {
        record CleanupAction(long address, long newSize,
                             Consumer<MemorySegment> cleanup) implements Runnable {
            @Override
            public void run() {
                cleanup().accept(_SegmentFactories.makeNativeSegmentUnchecked(address(), newSize()));
            }
        }
    ...
    @AlwaysInline
    private boolean checkBounds(long offset, long length) {
        return length >= 0 && offset >= 0 && offset <= this.length - length;
    }
    ...
    private void throwOutOfBounds(long offset, long length, boolean isSlice) {
        ...
        String msg = String.format(
                "Out of bound access on segment %s; attempting to %s of length %d at offset %d " +
